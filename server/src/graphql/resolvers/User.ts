...
import { Resolver, Mutation, Arg, Query, Authorized, Ctx, Int } from "type-graphql";
...
export class UserResolver{
  ...
  private sendResetPasswordEmail = async (token: string, email: string) => {
    ...
    const updated = insertParams(html, { resetLink: process.env.NODE_ENV === "production" ? `https://ammarahmed.ca/chess/reset-password/${token}` : `http://localhost:3000/chess/reset-password/${token}`});
    await this.mailer.sendMail({
      from: "Ammar Ahmed <ammar@ammarahmed.ca>",
      to: email,
  ...
  async confirmEmail(
    ...
    if (!user.emailConfirmationCode || !user.emailConfirmationCodeExpire) throw new Error("No code.");

    if (user.emailConfirmationCode !== code) throw new Error("Invalid code.");
    if (user.emailConfirmationCodeExpire <= new Date(Date.now())) throw new Error("Code expired.");
  ...
  async resetPassword(
    ...
    if (user.resetPasswordExpire && user.resetPasswordExpire <= new Date()) throw new Error("Token expired.")
