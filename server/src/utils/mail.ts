...
export const readHTML = (relativePath: string) : string => {
  return fs.readFileSync(path.resolve(__dirname, relativePath), { encoding: "utf-8" });
}
 ...
 */
export const insertParams = (htmlString: string, params: Record<string, any>) : string => {
  let copy = htmlString;
  const paramNames = getParamNames(htmlString);
  if (!paramNames) throw new Error("No params")
  Object.keys(params).forEach(param => {
    if (!paramNames.includes(param)) throw new Error(`param: ${param} does not exist in html.`)
