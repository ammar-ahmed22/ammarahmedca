...
export const readBlockContent = (blocks: (PartialBlockObjectResponse | BlockObjectResponse)[]) : IContent[] | undefined => {
        ...
        return {
          ...
          content: block.heading_1.rich_text.map( text => {
            return {
              ...
              annotations: {...text.annotations, color: text.annotations.color as string}
            }
          })
        }
      case "heading_2":
      ...
      case "code":
        return {
          ...
          content: block.code.rich_text.map( text => {
            return {
              plainText: text.plain_text,
              annotations: { ...text.annotations, language: block.code.language }
            }
          })
        }
