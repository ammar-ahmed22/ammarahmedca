    ...
    <Menu closeOnSelect={false}>
      ...
      <MenuList>
        <MenuOptionGroup
          ...
          type="radio"
          defaultValue="type"
          onChange={(value) => setFilterBy(value as keyof ProjectFilterOpts)}
        >
        ...
        {data && filterBy && filterParams && (
          <MenuOptionGroup
            title={filterBy[0].toUpperCase() + filterBy.substring(1)}
            type="checkbox"
          ...
          >
            {data.filterOpts[filterBy].map((item, idx) => {
              return (
                <MenuItemOption value={item} key={`${filterBy}-${idx}`}>
                  {item}
                </MenuItemOption>
              );
            })}
