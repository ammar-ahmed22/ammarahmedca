    ...
    >
      ...
      <Skeleton className="h-4 w-2/5 mb-4 rounded-full" />
      {
        new Array(5).fill(0).map((_, idx) => {
          return (
            <Skeleton key={`proj-skel-desc-${idx}`} className={`h-3 ${idx < 4 ? "w-100 mb-2" : "w-2/5 mb-4"} rounded-full`} />
          )
        })
      }
      <Skeleton className="h-2 w-1/5 mb-2 rounded-full" />
...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const start = format(new Date(project.dateRange.start), "LLLL, yyyy");
  const end = project.dateRange.end ? format(new Date(project.dateRange.end), "LLLL, yyyy") : "Present";
  return (
    <Card
    ...
    >
      <CardHeader
        className="pb-0 pt-2 px-8 flex items-start justify-between"
      >
        <div>
      ...
      <CardBody
        className="overflow-visible py-2 px-8 flex flex-col items-start space-y-2"
      >
