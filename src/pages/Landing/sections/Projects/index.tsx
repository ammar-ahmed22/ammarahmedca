...
import { PROJECT_METADATA_QUERY, ProjectMetadataQuery } from "../../../../graphql/queries/Metadata";
import { PROJECT_FILTER_OPTIONS_QUERY, ProjectFilterOptionsQuery } from "../../../../graphql/queries/FilterOpts";
import { Button, SelectProps, Skeleton, Card } from "@nextui-org/react";
import { ArrowDownIcon, ArrowUpIcon, } from "@heroicons/react/24/solid"
import { CodeBracketIcon, WrenchScrewdriverIcon, TagIcon } from "@heroicons/react/24/outline"
import { usePaginatedArray } from "../../../../hooks/paginated";
import Filter from "../../../../components/Filter";
import ProjectCard, { ProjectCardSkeleton } from "./ProjectCard";

const Projects: React.FC = () => {
  const gradient = useTextGradient({ dir: "r", from: "primary-500", to: "secondary-300" });
  const [projects, setProjects] = useState<IProjectMetadata[]>([]);
  const [getProjectMetadata, { data, loading, error }] = useLazyQuery<ProjectMetadataQuery.Response, ProjectMetadataQuery.Variables>(PROJECT_METADATA_QUERY);
  const filterOptsResp = useQuery<ProjectFilterOptionsQuery.Response>(PROJECT_FILTER_OPTIONS_QUERY);
  const [paginatedProjects, { increase, hasMore, reset }] = usePaginatedArray(projects);
  ...
  const hasFilters = useMemo(() => {
    return languageFilters.length > 0 || frameworkFilters.length > 0 || typeFilters.length > 0;
  ...
  }, [getProjectMetadata, languageFilters, typeFilters, frameworkFilters])
  ...
  return (
    <section
      className="transition relative my-3"
    >
      <h2
            ...
            <>
              {
                filterMaps.map(map => {
                  return (
                    <Filter 
                      options={filterOptsResp.data![map.key].map(o => ({ label: o, value: o }))}
                      multiSelect
                      includeClear
                      size="sm"
                      ...
                      onSelectionChange={(values) => map.setFilterValues(values)}
                    />
                  )
                })
              }
      ...
      <div className="grid md:grid-cols-2 grid-cols-1 gap-4">
        ...
        {
          projects.length > 0 && (hasFilters ? projects : paginatedProjects).map((project) => {
            return (
              <ProjectCard 
