...
export type UsePaginatedArrayResponse<T> = [T[], { increase: () => void, decrease: () => void, hasMore: boolean, reset: () => void }]

export function usePaginatedArray<T = any>(array: T[], opts?: UsePaginatedArrayOpts): UsePaginatedArrayResponse<T> {
  if (!opts) opts = {};
  ...
  const [paginated, setPaginated] = useState<T[]>(array.slice(0, length));
