...
const ScrollIndicator: React.FC<ScrollIndicatorProps> = ({ scrollToId }) => {
    ...
    if (circle.current && main.current) {
      if (scrollPos !== -1 && scrollPos > 0) {
        ...
        circle.current.style.animation =
          "scroll-down 2s ease forwards, fade-out 2s ease forwards";
      } else {
        circle.current.style.animation =
          "scroll-up 2s ease forwards, fade-in 2s ease forwards";
  ...
  const handleClick = () => {
    ...
    if (elToScrollTo && header) {
      ...
      const scrollDist =
        window.scrollY +
        elToScrollTo.getBoundingClientRect().top -
        navBarHeight;
  ...
  return (
    ...
    >
      <Box
        {...styles.ellipse}
        onMouseOver={handleScrollIndOver}
        onMouseOut={handleScrollIndOut}
        onClick={handleClick}
        ref={ellipse}
      >
        <Box {...styles.circle} ref={circle}></Box>
      </Box>
      <Text fontSize="xs" color="brand.purple.500">
        Scroll
      </Text>
    </Flex>
  );
