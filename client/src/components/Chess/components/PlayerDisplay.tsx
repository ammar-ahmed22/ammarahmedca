...
const PlayerDisplay : React.FC<PlayerDisplayProps> = ({ player, takes, takesColor, containerProps }) => {
  ...
  const renderStatHelpText = (takes: { w: Piece[], b: Piece[] }, takesColor: "w" | "b") => {
    const oppTakesColor = takesColor === "w" ? "b" : "w";
    const diff = totalPoints(takes[takesColor]) - totalPoints(takes[oppTakesColor]);
  ...
  return (
    <HStack w={`calc(8 * ${squareSize})`} {...containerProps} >
      <Box>
        <Text fontSize="lg">{player.firstName} {player.lastName}</Text>
        <Stat>
          
          <HStack>
          <StatLabel>Points</StatLabel>
            <StatNumber fontSize="sm">{totalPoints(takes[takesColor])}</StatNumber>
            <StatHelpText>
              {
                renderStatHelpText(takes, takesColor)
              }
            </StatHelpText>
          </HStack>
        </Stat>
