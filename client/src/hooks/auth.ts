...
export const useAuthMutation = <VariableType>(mutationName: AuthMutations, mutation: DocumentNode) : [MutationFunction<AuthMutations, VariableType>, { error: ApolloError | undefined, loading: boolean, submitted: boolean }] => {
  ...
  const [call, { data, loading, error }] = useMutation<Record<AuthMutations, AuthPayload>, VariableType>(mutation)
  ...
  const mutationFunction = (options?: MutationFunctionOptions<Record<AuthMutations, AuthPayload>, VariableType, DefaultContext, ApolloCache<any>>) => {
    call(options)
    setSubmitted(true);
  }
...
export const useGetUser = () : { user?: User, loading: boolean, error?: ApolloError } => {
  const userQuery = gql`
    query User {
      user {
        _id
