...
import { BlogMetadataQuery, BlogMetadataQueryResponse, BlogMetadataQueryVariables } from '../graphql/queries/Metadata';
...
const Blog : React.FC = () => {
    ...
    const { data, loading } = useQuery<BlogMetadataQueryResponse, BlogMetadataQueryVariables>(BlogMetadataQuery, { variables: { publishedOnly: true }});
    ...
    return (
        <>
                ...
                {
                    data && Object.keys(mapCategory(data.blogMetadata)).map( (category, catIdx) => {
                            ...
                            <Box  key={catIdx} width={"100%"} >
                                <Text {...styles.category} >{category}</Text>
                                {
                                    sortByDate(posts).map( (post, postIdx) => {
                                        const { name, readTime, description, published, pathname } = post;
                                        return (
                                            <Card isLink to={`/blog/${pathname as string}`} key={postIdx} >
                                                <Text {...styles.postTitle} >{name}</Text>
                                                <Text {...styles.postInfo} >{helper.displayTimeSince(published as number)} &bull; {readTime} min read</Text>
